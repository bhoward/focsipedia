...
## Section 4. Format
`image` type is formatted using the `Styled` function. In addition to an Image img, `Styled` function take a list as its argument. Element of this list can be thes following constructors: 

| `Styled`'s list element  | Argument()| Return |
| :-: | :-: | :-: |
| `LineWidth(float)` | A float | Image whose border's thickness is specified. |
| `LineColor(color)` | A `color` type | Image whose border's color is specified. |
| `FillColor(color)` | A `color` type | Image whose border's inside color is specified. |
| `Dashed` | None | Image with dashed border |

`color` is a type that can be generated by one of the following constructor: 
* `Color(string)`: Take a named CSS Level 4 colors (Figure~\ref{colfig}) as its argument. 
* `RGBA(int, int, int, float)`: Create a color with the given red, green, and blue components. The first 3 arguments should be a float between 0 and 255. The fourth argument is `alpha` level, which determines the opacity of the color, which should be a float between 0 and 1. An alpha of 1.0 is fully opaque, while 0.0 is fully transparent.
* `HSLA(angle, float, float, float)`: Create a color with the given hue, saturation, and lightness components. The first argument (hue) should be a float from 0 to 360, representing an angle in the color wheel (0 is red, 120 is green, and 240 is blue). The second argument (saturation) should be a float from 0 to 1, measuring of how pure the hue is (1 is fully pure, while 0 is a shade of gray). The third argument (lightness) should be a float from 0 to 1, measuring of how close to white (1) or black (0) it is. For example, a fully pure blue is represented by (240., 1., 0.5) combination of (hue, saturation, and lightness). The last argument is the alpha value, as in RGBA.
...
yellowGreen
\end{tabular}}
\caption{CSS Level 4 Color Names}
\label{colfig}
\end{center}
\end{figure}
