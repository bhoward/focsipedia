...
| `style` | Argument | Effect |
| :-: | :-: | :-: |
| `LineWidth(lineThickness)` | Line thickness, which is a float | Use the specified border thickness. |
...
| `Font(fontSize, fontFamily, fontWeight, fontStyle)` | Font Size (which is a float), font family (`fontFamily` type) , font weight, font style | Format the text as specified|

Constructor `LineColor(color)` and `FillColor(color)` take type `color` as its argument. The type `color` can be generated by one of the following constructors: 
