...
and false. We will write these values as $\mathbb{T}$ and $\mathbb{F}$.
The symbols $\mathbb{T}$ and $\mathbb{F}$ play a similar role in Boolean algebra
...
by interchanging $\land$ with $\lor$ and $\mathbb{T}$ with $\mathbb{F}$. This cuts down
...
obtained from it by interchanging $\land$ with $\lor$ and $\mathbb{T}$ with $\mathbb{F}$.
...
| Excluded middle   | $p\lor\lnot p\equiv\mathbb{T}$                    |
| Contradiction     | $p\land\lnot p\equiv\mathbb{F}$                   |
| Identity laws     | $\mathbb{T}\land p\equiv p$                       |
...
that two negatives make a positive. Although this rule may be questionable
when applied to English as it is actually used&mdash;no matter what the
...
| | $\equiv \mathbb{F}\lor(p\land q)$ | Law of Contradiction, 2nd Subst. Law |
...
Substitution Law to the Identity Law, $\mathbb{F}\lor p\equiv p$, to obtain
$\mathbb{F}\lor(p\land q)\equiv (p\land q)$. 
...
of saying the same thing is that $P$ is a tautology if $P\equiv\mathbb{T}$.
So, we can prove that a compound proposition, $P$, is a tautology by
finding a chain of logical equivalences leading from $P$ to $\mathbb{T}$.
For example:
...
| | $\equiv \mathbb{T}$ | Law of Excluded Middle |
...
table asserts only that $\mathbb{F}\lor p\equiv p$, it is also true that
$p\lor\mathbb{F}\equiv p$. This can be checked directly or by a simple calculation:

| | | |
| -: | :- | :- |
| $p\lor\mathbb{F}$ | $\equiv \mathbb{F}\lor p$ | Commutative Law |
...
and false when $p$ is false. That is, $p\lor\mathbb{F}$ has the same logical
value as $p$. But that's just what the Identity Law for $\lor$ says.
A few of the laws need more discussion.

The Law of the Excluded Middle, $p\lor\lnot p\equiv \mathbb{T}$,
...
$p\land\lnot p\equiv\mathbb{F}$, says that it is not possible for 
...
5. In the algebra of numbers, there is a distributive
...
two expression $p\land\mathbb{F}$ and $p\lor\mathbb{T}$. What are the missing laws?
...
functionally complete, provided the constant $\mathbb{F}$ is also available (this is not
unreasonable in an electronic circuit, where $\mathbb{F}$ is represented by the ground
voltage). That is, show how to define $p\land q$, $p\lor q$, and $\lnot p$ by
giving equivalent expressions using only $\rightarrow$ and $\mathbb{F}$ in addition to
